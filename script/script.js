...
$(function () {
  ...
  $("#fst .encrypt").click(function () {
    body.attr("class", "encrypt");
    step(2);
  });

  $("#fst .decrypt").click(function () {
  ...
  var file = null;
  $("#snd").on("change", "#encrypt-input", function (e) {
  ...
  $("#snd").on("change", "#decrypt-input", function (e) {
    if (e.target.files.length != 1) {
    ...
    if (body.hasClass("encrypt")) {
      ...
      reader.onload = function (e) {
        ...
        if (ciphertext.charAt(0) === "1") {
          ...
          var decrypted = CryptoJS.AES.decrypt(ciphertext, password).toString(
            CryptoJS.enc.Latin1
          );
        ...
        if (ciphertext.charAt(0) === "2") {
          ...
          var decrypted = CryptoJS.DES.decrypt(ciphertext, password).toString(
            CryptoJS.enc.Latin1
          );
          if (!/^data:/.test(decrypted)) {
            alert("Invalid file! Please try again.");
  ...
  back.click(function () {
    $("#snd input[type=file]").replaceWith(function () {
      return $(this).clone();
    });
    step(1);
  });
